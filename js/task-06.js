...
const validNumberOfSymbols = inputValid.dataset.length;

inputValid.addEventListener("blur", validationInputHandler);

function validationInputHandler(event) {
  if (inputValid.value.length >= validNumberOfSymbols) {
    inputValid.classList = "valid";
  } else if (inputValid.value.length < validNumberOfSymbols) {
